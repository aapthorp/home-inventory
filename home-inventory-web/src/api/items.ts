...
import type { Item, ItemRequest, UUID } from "@/types/inventory";
...
async function createItem(payload: ItemRequest): Promise<Item> {
  const { data } = await apiClient.post<Item>("/items", payload);
  return data;
}

async function updateItem(id: UUID, payload: Partial<ItemRequest>): Promise<Item> {
...
export function useItems(filters: ItemFilters = {}) {
  return useQuery({ queryKey: ["items", filters], queryFn: () => fetchItems(filters) });
}
...
export function useUpdateItem(id: UUID) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (payload: Partial<ItemRequest>) => updateItem(id, payload),
    onSuccess: () => {
